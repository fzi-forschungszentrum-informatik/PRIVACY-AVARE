...
void hookAndroidVM(JArrayClass<jobject> javaMethods,
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla10");

    JNIEnv *env = Environment::current();
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "env: %s", env);

    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%i", env->GetVersion());

    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla11");


    JNINativeMethod methods[] = {
            NATIVE_METHOD((void *) mark, "nativeMark", "()V"),
    };
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla12");
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", methods->name);
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%i", nativeEngineClass.get() == nullptr);

    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla13");


    if (env->RegisterNatives(nativeEngineClass.get(), methods, 1) < 0) {
        __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla14");

        __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "register natives: %i", env->RegisterNatives(nativeEngineClass.get(), methods, 1));
        __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla15");
        return;
    }
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla16");
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla17");

    void *soInfo = getDvmOrArtSOHandle();
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla18");
...
void *getDvmOrArtSOHandle() {
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla20");
    if (strlen(so_name) == 0) {
        __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla21");
        __system_property_get("persist.sys.dalvik.vm.lib", so_name);
    }
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla22");
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "so_name: %s", so_name);
    void *soInfo = dlopen(so_name, 0);
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla220");
    if (!soInfo) {
        __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla23");
        soInfo = RTLD_DEFAULT;
    }
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla24");
