...
#include "../../../../../YAHFA/src/main/jni/env.h"
...
HOOK_DEF(int, execve, const char *pathname, char *argv[], char *const envp[]) {
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla300");
    if (strstr(pathname, "dex2oat")) {
        //fzimerge
        char **new_envp = build_new_env(envp);__android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla301");
        __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "redirect_path: %s", redirect_path);
        __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "new_envp: %s", new_envp);
        int ret = syscall(__NR_execve, redirect_path, argv, new_envp);
        __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla302");
        FREE(redirect_path, pathname);
        return ret;
    }
    int ret = syscall(__NR_execve, redirect_path, argv, envp);
...
void hook_dlopen(int api_level) {
    __android_log_print(ANDROID_LOG_VERBOSE, "VMPatch", "%s", "bla400");
    void *symbol = NULL;
    if (api_level > 23) {
